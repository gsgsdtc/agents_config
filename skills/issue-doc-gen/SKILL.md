...
| 允许写入 | 禁止写入 |
|----------|----------|
| 用户场景（含前置/后置/异常）、业务流程 | 代码片段、伪代码 |
| 业务实体（概念和关系，不写 schema） | API 设计、数据库 schema、字段定义 |
| 业务规则（跨场景的校验和约束） | 具体的技术方案、架构设计 |
| ASCII 原型图（用户视角的页面布局和交互流程） | 组件结构、UI 框架细节 |
| 验收标准（可测试的行为描述，标注场景追溯） | 函数签名、类定义、SQL |
| 约束条件、依赖关系 | 实现步骤、代码修改清单 |
